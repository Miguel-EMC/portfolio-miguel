    ...
    <div class="login-page animate-fade-in">
      <div class="login-card">
        <header class="login-header">
          ...
          <p>{{ 'auth.signup.subtitle' | translate }}</p>
        </header>

        <form (ngSubmit)="onSignup()" class="login-form">
          <div class="form-group">
            <label for="name">{{ 'auth.fields.name' | translate }}</label>
            <input type="text" id="name" name="name" [placeholder]="'auth.placeholders.name' | translate" class="form-control" />
          </div>
          ...
          <button type="submit" class="btn-primary">{{ 'auth.signup.action' | translate }}</button>
        </form>

        <footer class="login-footer">
          <p>{{ 'auth.signup.hasAccount' | translate }} <a routerLink="/auth/login">{{ 'auth.signup.login' | translate }}</a></p>
...
export class CommunitySignupComponent {
  private router = inject(Router);
  onSignup() { this.router.navigate(['/auth/login']); }
}
